...
class sce:    #class that get the scenario from the file
    ...
    def __init__(self,path1,def_name,jalaseh):
        ...
        self.estenbat=dict() 
        data_folder="../scenarios/amoozeshi/"+jalaseh+"/"
...
def shoro(client_data):
        ...
        logininterface.button_1.configure(command=lambda:insert(logininterface.entry_1.get(),client_data))
                    ...
                    if vorod_dasti==False :
                        ...
                        if last_j=="pish":
                            ...
                            break
                             ...
                             messagebox.showinfo("هشدار","شما پس آزمون را طی کرده و قادر به طی کردن مراحل آموزشی خودکار نیستید\nصرفا میتوانید به صورت دستی وارد شوید")
                             next_page=False
                             break
...
for reza in final_list :
    ...
    for i in range (0,len(list_of_sen)):  #show  main sentenses and simple question of each one
        ...
        bg=PhotoImage(file="../bak/bc1-1000.600.png")
        ...
        def move():
            ...
            if a<len(sc):
                ...
                a+=1
                root.after(5000,move) # time of each scense
            elif a==len(sc):
                global sq
                root.destroy() 
    ...
    bg=PhotoImage(file="../bak/bc1-800.500.png")
    bg_label=Label(root,image=bg)
    bg_label.place(x=0,y=0)
